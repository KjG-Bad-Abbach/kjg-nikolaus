...
export const scenarioCoverage = {
  ...
  resume: {
    ...
    testIds: ['qa-step-address', 'qa-contact-phone', 'qa-view-steps', 'qa-stepper'],
  },
  addressDeadline: {
  ...
  errorModal: {
    ...
    testIds: ['qa-error-modal', 'qa-error-retry', 'qa-error-dismiss', 'qa-error-details-toggle'],
  },
  fullJourney: {
...
const hasAnnotation = (info: TestInfo, key: ScenarioKey) =>
  info.annotations.some(
    (annotation) =>
      annotation.type === annotationType && typeof annotation.description === 'string' && annotation.description.startsWith(`${key}:`),
  );
